...
def _kinetic_step(wfn: SpinTwoWavefunction, pm: dict) -> None:
    ...
    """
    wfn.fourier_plus2_component *= xp.exp(-0.25 * 1j * pm["dt"] * wfn.grid.wave_number)
    wfn.fourier_plus1_component *= xp.exp(-0.25 * 1j * pm["dt"] * wfn.grid.wave_number)
    wfn.fourier_zero_component *= xp.exp(-0.25 * 1j * pm["dt"] * wfn.grid.wave_number)
    wfn.fourier_minus1_component *= xp.exp(-0.25 * 1j * pm["dt"] * wfn.grid.wave_number)
    wfn.fourier_minus2_component *= xp.exp(-0.25 * 1j * pm["dt"] * wfn.grid.wave_number)
...
def _density(wfn: SpinTwoWavefunction) -> xp.ndarray:
    return (
...
def _singlet_duo(wfn: SpinTwoWavefunction) -> xp.ndarray:
    return (
        1
        / xp.sqrt(5)
        * (
...
def _evolve_spin_singlet(
    wfn: SpinTwoWavefunction, dens: xp.ndarray, singlet: xp.ndarray, pm: dict
) -> list[xp.ndarray]:
    s = xp.nan_to_num(xp.sqrt(dens**2 - abs(singlet) ** 2))
    cos_term = xp.cos(pm["c4"] * s * pm["dt"])
    sin_term = xp.sin(pm["c4"] * s * pm["dt"]) / s
    sin_term[s == 0] = 0  # Corrects division by 0

    psi_p2 = (
        wfn.plus2_component * cos_term
        + 1j
        * (dens * wfn.plus2_component - singlet * xp.conj(wfn.minus2_component))
        * sin_term
    )
    psi_p1 = (
        wfn.plus1_component * cos_term
        + 1j
        * (dens * wfn.plus1_component + singlet * xp.conj(wfn.minus1_component))
        * sin_term
    )
    psi_0 = (
        wfn.zero_component * cos_term
        + 1j
        * (dens * wfn.zero_component - singlet * xp.conj(wfn.zero_component))
        * sin_term
    )
    psi_m1 = (
        wfn.minus1_component * cos_term
        + 1j
        * (dens * wfn.minus1_component + singlet * xp.conj(wfn.plus1_component))
        * sin_term
    )
    psi_m2 = (
        wfn.minus2_component * cos_term
        + 1j
        * (dens * wfn.minus2_component - singlet * xp.conj(wfn.plus2_component))
        * sin_term
    )
...
def _calculate_spin_vectors(wfn: list[xp.ndarray]):
    fp = xp.sqrt(6) * (wfn[1] * xp.conj(wfn[2]) + wfn[2] * xp.conj(wfn[3])) + 2 * (
        wfn[3] * xp.conj(wfn[4]) + wfn[0] * xp.conj(wfn[1])
    )
    fz = 2 * (abs(wfn[0]) ** 2 - abs(wfn[4]) ** 2) + abs(wfn[1]) ** 2 - abs(wfn[3]) ** 2
    return fp, fz


def _calc_q_factors(mod_f: xp.ndarray, pm: dict):
    cos1, sin1 = xp.cos(pm["c2"] * mod_f * pm["dt"]), xp.sin(
        pm["c2"] * mod_f * pm["dt"]
    )
    cos2, sin2 = xp.cos(2 * pm["c2"] * mod_f * pm["dt"]), xp.sin(
        2 * pm["c2"] * mod_f * pm["dt"]
    )
...
def _calc_qpsi(fz, fp, wfn):
    qpsi = [
        2 * fz * wfn[0] + fp * wfn[1],
        xp.conj(fp) * wfn[0] + fz * wfn[1] + xp.sqrt(3 / 2) * fp * wfn[2],
        xp.sqrt(3 / 2) * (xp.conj(fp) * wfn[1] + fp * wfn[3]),
        xp.sqrt(3 / 2) * xp.conj(fp) * wfn[2] - fz * wfn[3] + fp * wfn[4],
        xp.conj(fp) * wfn[3] - 2 * fz * wfn[4],
    ]
...
def _renormalise_wavefunction(wfn: SpinTwoWavefunction) -> None:
    ...
    wfn.plus2_component *= xp.sqrt(correct_atom_num / current_atom_num)
    wfn.plus1_component *= xp.sqrt(correct_atom_num / current_atom_num)
    wfn.zero_component *= xp.sqrt(correct_atom_num / current_atom_num)
    wfn.minus1_component *= xp.sqrt(correct_atom_num / current_atom_num)
    wfn.minus2_component *= xp.sqrt(correct_atom_num / current_atom_num)
    wfn.fft()
...
def _calculate_atom_num(
    ...
    """
    atom_num_plus2 = wfn.grid.grid_spacing_product * xp.sum(
        xp.abs(wfn.plus2_component) ** 2
    )
    atom_num_plus1 = wfn.grid.grid_spacing_product * xp.sum(
        xp.abs(wfn.plus1_component) ** 2
    )
    atom_num_zero = wfn.grid.grid_spacing_product * xp.sum(
        xp.abs(wfn.zero_component) ** 2
    )
    atom_num_minus1 = wfn.grid.grid_spacing_product * xp.sum(
        xp.abs(wfn.minus1_component) ** 2
    )
    atom_num_minus2 = wfn.grid.grid_spacing_product * xp.sum(
        xp.abs(wfn.minus2_component) ** 2
    )
